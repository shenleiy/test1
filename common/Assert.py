...
class Assertions:
    ...
    def assert_code(self, code, expected_code):
        ...
        try:
            ...
            self.logger.info("code状态码返回正确：, expected_code is %s, statusCode is %s " % (expected_code, code))
            # return True
        except Exception:
            self.logger.error("code状态码返回错误, expected_code is %s, statusCode is %s " % (expected_code, code))
    ...
    def assert_body(self, body, body_msg, expected_msg):
        ...
        try:
            ...
            self.logger.info(
                "message信息返回正确, expected_msg is %s, body_msg is %s" % (
                    expected_msg, body[body_msg]))
            # return True

        except Exception:
            self.logger.error(
                "message信息返回错误, expected_msg is %s, body_msg is %s" % (
                    expected_msg, body[body_msg]))
            Consts.RESULT_LIST.append('fail')
