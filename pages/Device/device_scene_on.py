...
class device_scene(Element.Element):
    def __init__(self, driver):
        ...
        device_scene.get_name(self, '开机')
        module = ['制冷', '除湿', '送风', '自动']
        Module = random.choice(module)
        print(Module)
        if device_scene.find_name(self, '制热'):
            device_scene.get_name(self, '制热')
            device_scene.get_name(self, '温度')
            device_scene.get_name(self, '确定')
            device_scene.get_name(self, '确定')
            print('制热模式开机26℃场景')
        else:
            print('单冷机型')
            device_scene.get_name(self, Module)
            if device_scene.find_name(self, '温度'):
                device_scene.get_name(self, '温度')
                device_scene.get_name(self, '确定')
                device_scene.get_name(self, '确定')
            else:
